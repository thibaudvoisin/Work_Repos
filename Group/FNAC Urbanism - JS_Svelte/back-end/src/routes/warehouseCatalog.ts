...
import { warehouseCatalogController } from '../controllers/warehouseCatalogController';
 ...
 *     tags:
 *       - Catalog
 *     summary: Get the warehouses catalog
 *     responses:
 *       200:
...
router.get('/catalog/warehouses', async function (_req, res, next) {
  try {
    await warehouseCatalogController(_req, res, next);
  } catch (error) {
    next(error);
  }
});

...
import { globalCatalogController } from '../controllers/globalCatalogController';
 ...
 *   get:
 *     tags : [Catalog]
 *     summary: Get the general catalog
 *     responses:
 *       200:
...
router.get('/catalog/global', async function (_req, res, next) {
  try {
    await globalCatalogController(_req, res, next);
  } catch (error) {
    next(error);
  }
});
